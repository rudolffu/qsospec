# qsospec Status and Development Plan

## Purpose

`qsospec` is the array-based NumPy/SciPy fitting core being developed
...
`qsospec.recipes` provides immutable `ComplexRecipe` and `ComponentRecipe`
...
- Keep the deprecated `NeoFit*` result names available through `qsospec` for
  the 0.1 release series.
