# qsospec Iron Templates

This directory contains normalized iron-template support for
`qsospec`. The optimizer uses plain NumPy arrays only; template file
...
```python
qsospec.IronTemplateConfig(template="park22", fwhm_kms=4000.0)
```

External two-column ASCII templates are still supported:

```python
qsospec.IronTemplateConfig(template="external", template_path="/path/to/template.txt")
```
