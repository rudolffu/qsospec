...
def _covariance_from_jacobian(
    ...
    reduced_chi2: float,
    names: Sequence[str],
) -> Tuple[Optional[np.ndarray], Dict[str, float], List[FitWarning]]:
...
def _active_bound_warnings(result, names: Sequence[str]) -> List[FitWarning]:
    warnings = []
...
def _append_workflow_warning(
    ...
    *,
    ...
    codes.append(code)
    warnings.append(
        FitWarning(code=code, message=message, severity=severity, context=context or {})
    )
