"""Tests for qsospec independent local fitting mode."""
...
def test_fit_local_two_independent_windows():
    ...
    config = qsospec.LocalFitConfig(windows=[qsospec.recipes.local_hbeta(), qsospec.recipes.local_halpha()])
...
def test_fit_local_window_outside_coverage_fails_without_crashing():
    spec = _two_window_spectrum()
    outside = qsospec.recipes.local_mgii()
    config = qsospec.LocalFitConfig(windows=[outside, qsospec.recipes.local_hbeta()])
