...
def test_recipe_registry_is_immutable_and_overrides_by_copy():
    ...
    changed = recipe.with_component(
        "CIV_broad", velocity_bounds_kms=(-4500.0, 2500.0)
    )
...
def test_continuum_only_has_no_hbeta_or_aggregate_summary_verdict():
    result = qsospec.fit_global_lines(
        _spectrum(),
        _continuum_config(
            qsospec.BalmerPseudoContinuumConfig(amplitude=12.0)
        ),
        complexes=[],
    )
    ...
    assert (
        result.metadata["balmer_pseudocontinuum_fwhm_source"]
        == "free_global_fit"
    )
...
def test_never_and_fixed_balmer_width_policies_do_not_synchronize():
    ...
    assert (
        never.metadata["balmer_pseudocontinuum_fwhm_source"]
        == "free_global_fit"
    )
    assert fixed.metadata["hbeta_sync_requested"] is False
    assert (
        fixed.metadata["balmer_pseudocontinuum_fwhm_source"]
        == "fixed_config"
    )
    assert fixed.metadata[
        "balmer_pseudocontinuum_fwhm_kms"
    ] == pytest.approx(4100.0)
...
def test_generic_uv_recipe_recovers_profile_metrics(recipe_name):
    ...
    continuum = GlobalContinuumResult(
        True, 1, "known", {}, {}, None, 0.0, 1, 0.0,
        wave.copy(), continuum_model, {"power_law": continuum_model},
        spectrum.valid_mask.copy(), spectrum.valid_mask.copy(),
    )
...
def test_balmer_components_follow_maximum_rest_wavelength(maximum, disabled):
    ...
    assert (
        "balmer_components_disabled_short_coverage"
        in result.continuum.warning_codes()
    ) is disabled
